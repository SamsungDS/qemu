...
static bool nvme_determine_rgif(uint16_t nruh, uint16_t nrg, uint8_t *rgif)
{
    uint16_t val;
    unsigned int i;
...
static bool nvme_subsys_setup_fdp(NvmeSubsystem *subsys, Error **errp)
{
    ...
    if (!nvme_determine_rgif(endgrp->fdp.nruh, endgrp->fdp.nrg, &endgrp->fdp.rgif)) {
        error_setg(errp, "cannot derive a valid rgif (nruh %"PRIu16" nrg %"PRIu32")",
