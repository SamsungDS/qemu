...
static const uint32_t nvme_cse_iocs_compute[256] = {
    [NVME_CMD_FLUSH]                = NVME_CMD_EFF_CSUPP | NVME_CMD_EFF_LBCC,
    [NVME_CMD_EXECUTE_PROGRAM]      = NVME_CMD_EFF_CSUPP,
};
...
typedef struct NvmeSLMCopyAIOCB {
    BlockAIOCB common;
    BlockAIOCB *aiocb;
    NvmeRequest *req;
    NvmeCtrl *n;
...
static void nvme_slm_copy_cancel(BlockAIOCB *aiocb)
{
    NvmeSLMCopyAIOCB *iocb = container_of(aiocb, NvmeSLMCopyAIOCB, common);
...
static const AIOCBInfo nvme_slm_copy_aiocb_info = {
    .aiocb_size   = sizeof(NvmeSLMCopyAIOCB),
    .cancel_async = nvme_slm_copy_cancel,
};
...
static void nvme_slm_copy_done(NvmeSLMCopyAIOCB *iocb)
{
    NvmeRequest *req = iocb->req;
...
static void nvme_copy_source_range_parse_format4(void *ranges,
                                                 int idx,
                                                 uint64_t *slba,
                                                 uint32_t *nlb,
                                                 uint32_t *snsid)
{
    NvmeCopySourceRangeFormat4 *_ranges = ranges;

    if (slba) {
        *slba = le64_to_cpu(_ranges[idx].saddr);
    }

    if (nlb) {
        *nlb = le32_to_cpu(_ranges[idx].nbyte);
    }

    if (snsid) {
        *snsid = le32_to_cpu(_ranges[idx].snsid);
    }
                                         ...
                                         uint16_t *appmask, uint64_t *reftag)
{
    ...
    case NVME_COPY_FORMAT_4:
        nvme_copy_source_range_parse_format4(ranges, idx, slba, nlb, snsid);
        break;
...
static void nvme_copy_out_completed_cb(void *opaque, int ret)
{
    ...
    if (iocb->format == NVME_COPY_FORMAT_4) {
        iocb->copy_length -= nlb;
        nlb = nvme_b2l(dns, nlb);
    }
    iocb->idx++;
    iocb->slba += nlb;
out:
...
static void nvme_slm_copy_out_cb(void *opaque, int ret);

static void nvme_slm_do_copy(NvmeSLMCopyAIOCB *iocb)
{
    NvmeRequest *req = iocb->req;
    NvmeNamespace *ns = req->ns;
    ...
    case NVME_COPY_FORMAT_3:
        ...
        iocb->aiocb = blk_aio_preadv(ns->blkconf.blk, nvme_l2b(ns, slba),
                                 &iocb->iov, 0, nvme_slm_copy_out_cb, iocb);
        break;
    case NVME_COPY_FORMAT_4:
        ...
        if (slm_ns->params.slm) {
            ...
            break;
    default:
            printf("%s: Invalid descriptor format\n", __func__);
            goto invalid;
    }
...
invalid:
    ...
    nvme_slm_copy_done(iocb);
}

static void nvme_slm_copy_out_cb(void *opaque, int ret)
{
    NvmeSLMCopyAIOCB *iocb = opaque;
    NvmeRequest *req = iocb->req;
    NvmeNamespace *ns = req->ns;
    uint32_t nlb = 0;
    uint64_t slba = 0;
    ...
    case NVME_COPY_FORMAT_3:
        ...
        if ((iocb->sdaddr_offset >= iocb->dns->size) ||
           ((iocb->sdaddr_offset + len) > iocb->dns->size)) {
            req->status = NVME_CMD_SIZE_LIMIT | NVME_DNR;
            iocb->ret = -1;
            goto out;
        }

        memcpy(&iocb->dns->slm_buf[iocb->sdaddr_offset],
               (void *)iocb->bounce, len);
        iocb->copy_length -= len;
        iocb->idx++;
        iocb->sdaddr_offset += len;
        break;
    case NVME_COPY_FORMAT_4:
        iocb->copy_length -= nlb;
        nlb = nvme_b2l(iocb->dns, nlb);
        iocb->idx++;
        iocb->slba += nlb;
        break;
    }
...
static void nvme_do_copy_fmt4(NvmeCopyAIOCB *iocb)
{
    NvmeNamespace *slm_ns = NULL;
    NvmeRequest *req = iocb->req;
    NvmeNamespace *ns = req->ns;
    ...
    nvme_copy_source_range_parse_format4(iocb->ranges, iocb->idx,
                                         &slba, &nlb, &snsid);
...
static uint16_t nvme_copy(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    if (format == NVME_COPY_FORMAT_4) {
        ...
        len = (
            ns->pif ?
            sizeof(NvmeCopySourceRangeFormat1_3) :
            sizeof(NvmeCopySourceRangeFormat0_2)
        );
...
static uint16_t nvme_compute_exec_prgm(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    uint32_t nsid = le32_to_cpu(ep->nsid);
    NvmeNamespace *ns = nvme_ns(n, nsid);
    uint16_t pind = le16_to_cpu(ep->pind);
    ComputeProgram *program = &(ns->compute_prgm_arr[pind]);
    uint64_t len = le64_to_cpu(ep->cparam2);
    uint16_t status;
    uint64_t return_res = 0;
    g_autofree uint8_t *buf = NULL;
    g_autofree memory_range_ds *ranges = NULL;
    NvmeMemoryRangeSet *mrs1 = NULL;
    char *host_temp_path = ns->params.host_temp_path;
    int n_htp = strlen(host_temp_path);

    if ((program->prgm_discr.peocc != 1) && (program->prgm_discr.peocc != 2)) {
        return NVME_NO_PROGRAM;
    }

    if (program->prgm_discr.act == 0) {
        return NVME_PROGRAM_NOT_ACTIVE;
    }

    if ((ep->rsid == 0) && (ep->numr == 0)) {
        buf = g_malloc(len);
        status = nvme_h2c(n, buf, len, req);
        if (status) {
            return status;
        }
    }
    if ((ep->rsid == 0) && (ep->numr != 0)) {
        ranges =  g_malloc0(ep->numr * sizeof(memory_range_ds));

        status = nvme_h2c(n, (uint8_t *)ranges, len, req);
        if (status) {
            return status;
        }

        if (program->prgm_discr.ptype == 0xC0 ||
            program->prgm_discr.ptype == 0xC1) {
            uint64_t data_len = le64_to_cpu(ep->cparam1);
            buf = g_malloc0(data_len);
        } else{
            uint64_t total_len = 0;
            for (int i = 0; i < ep->numr; i++) {
                total_len += ranges[i].len;
            }
    ...
    if (ep->rsid > 0) {
        ...
        if (program->prgm_discr.ptype == 0xC0 ||
            ...
            uint64_t total_len = 0;
            for (int i = 0; i < mrs1->num_mrd; i++) {
                total_len += mrs1->mr[i].length;
            }
            buf = g_malloc0(total_len);
        }
    }
    ...
    case 0xC1:
    {
        int offset = 0;
        if (ep->rsid == 0 && ep->numr != 0) {
            for (int i = 0; i < ep->numr - 1; i++) {
                if (nvme_namespace_reachable(n, nsid, ranges[i].mnsid)) {
                    NvmeNamespace *m_ns = nvme_ns(n, ranges[i].mnsid);
                    memcpy(buf + offset,
                           (uint8_t *)&m_ns->slm_buf[ranges[i].sb],
                           ranges[i].len);
                    offset += ranges[i].len;
                } else {
        ...
        if (ep->rsid > 0) {
            for (int i = 0; i < mrs1->num_mrd - 1; i++) {
                NvmeNamespace *m_ns = nvme_ns(n, mrs1->mr[i].mnsid);
                memcpy(buf + offset,
                       (uint8_t *)&m_ns->slm_buf[mrs1->mr[i].saddr],
                       mrs1->mr[i].length);
                offset += mrs1->mr[i].length;
            }
        }
        g_autofree char *str = g_new(char, 70 + n_htp);
        g_autofree uint8_t *temp_buf = NULL;

        sprintf(str, "gcc -no-pie %sn-%d_prgm-%d.o -o tt && ./tt '",
                        host_temp_path, nsid, pind);
        int prefix_len = strlen(str);

        g_autofree char *out_str = g_new(char, 50 + n_htp);
        sprintf(out_str, " > %sn-%d_prgm-%d.txt",
                host_temp_path, nsid, pind);

        temp_buf = g_malloc(prefix_len + len + strlen(out_str));

        strcpy((char *)temp_buf, str);
        int buf_size = strlen((char *)buf);
        strcat((char *)temp_buf, (char *)buf);
        temp_buf[prefix_len + buf_size - 1] = 39;
        strcat((char *)temp_buf, out_str);

        int ret_val = system((char *)temp_buf);
        if (ret_val == -1) {
            return -1;
        }

        memset(out_str, '\0', 50 + n_htp);
        sprintf(out_str, "%sn-%d_prgm-%d.txt", host_temp_path, nsid, pind);

        int fd1 = open(out_str, O_RDONLY | O_CREAT, 00700);
        if (fd1 < 0) {
            printf("error opening file\n");
            return -1;
        }

        char *Buf1 = g_new(char, 20);
        ssize_t ret_bytes = read(fd1, Buf1, 20);
        if (ret_bytes < 0) {
        ...
        char *buf2 = g_new(char, ret_bytes);
        strcpy(buf2, Buf1);
        return_res = atoll(Buf1);
        printf("CRC: %lu\n", return_res);

        if ((ep->rsid == 0) && (ep->numr != 0)) {
            if (nvme_namespace_reachable(n, nsid,
                ranges[ep->numr - 1].mnsid)) {
                NvmeNamespace *m_ns = nvme_ns(n, ranges[ep->numr - 1].mnsid);
                memcpy((uint8_t *)&m_ns->slm_buf[ranges[ep->numr - 1].sb],
                        &return_res, ranges[ep->numr - 1].len);
            } else {
                printf("%s: memory namespace not reachable\n", __func__);
                status = NVME_INVALID_MEMORY_RANGE_SET | NVME_DNR;
                return status;
            }
        }
        if (ep->rsid > 0) {
            NvmeNamespace *m_ns = nvme_ns(n,
                                          mrs1->mr[mrs1->num_mrd - 1].mnsid);
            memcpy((uint8_t *)&m_ns->slm_buf[mrs1->mr[mrs1->num_mrd - 1].saddr],
                    (void *)Buf1, mrs1->mr[mrs1->num_mrd - 1].length);
        }
        req->cqe.result = return_res & 0xFFFFFFFF;
        req->cqe.dw1 = ((return_res >> 32) & 0xFFFFFFFF);
        return NVME_SUCCESS;
    }
    break;
    default:
...
static uint16_t nvme_slm_copy(NvmeCtrl *n, NvmeRequest *req)
{
    uint16_t ret = NVME_SUCCESS;
    NvmeSLMCopyCmd *slm_copy_cmd = (NvmeSLMCopyCmd *)&req->cmd;
    NvmeNamespace *dest_ns = req->ns;
    uint64_t starting_byte = le64_to_cpu(slm_copy_cmd->starting_address);
    uint32_t dw2 = le32_to_cpu(slm_copy_cmd->cdw2);
    uint32_t dw3 = le32_to_cpu(slm_copy_cmd->cdw3);
    uint64_t copy_length = ((uint64_t)dw3) << 32 | dw2;
    int64_t remaining_size = copy_length;
    uint64_t sb_offset = starting_byte;
    uint8_t nr = le32_to_cpu(slm_copy_cmd->nr) + 1;
    uint8_t desc_format = leul_to_cpu(slm_copy_cmd->desc_format);
    size_t len = 0;
    NvmeNamespace *ns = NULL;
    NvmeSLMCopyAIOCB *iocb =  NULL;

    if (nr > dest_ns->id_ns.msrc + 1) {
        ret = NVME_CMD_SIZE_LIMIT | NVME_DNR;
        goto exit;
    }

    if (!(n->id_ctrl.ocfs & (1 << desc_format))) {
        trace_pci_nvme_err_copy_invalid_format(desc_format);
        ret = NVME_INVALID_FIELD | NVME_DNR;
        goto exit;
    }

    if (copy_length > dest_ns->params.slm_mcl) {
        ret = NVME_CMD_SIZE_LIMIT | NVME_DNR;
        goto exit;
    }
    ...
    switch (desc_format) {
    case NVME_COPY_FORMAT_4:
        NvmeCopySourceRangeFormat4 *sranges = NULL;
        len = sizeof(NvmeCopySourceRangeFormat4);
        sranges = g_malloc0((nr * len));
        ret = nvme_h2c(n, (uint8_t *)sranges, len * nr, req);
        if (ret) {
                goto exit;
        }

            for (int i = 0; i < nr; i++) {
                ns = nvme_ns(n, sranges[i].snsid);
                if (ns) {
                    if (ns->params.slm) {
                        int64_t copy_size;
                        if (remaining_size <= 0) {
                        ...
                        if ((sb_offset >= dest_ns->size) ||
                           ((sb_offset + copy_size) > dest_ns->size)) {
                            g_free(sranges);
                            return NVME_CAP_EXCEEDED | NVME_DNR;
                        }

                        if ((sranges[i].saddr >= ns->size) ||
                            ((sranges[i].saddr + copy_size) > ns->size)) {
                            g_free(sranges);
                            return NVME_CAP_EXCEEDED | NVME_DNR;
                        }

                        if (copy_size > dest_ns->id_ns.mssrl) {
                            ret = NVME_CMD_SIZE_LIMIT | NVME_DNR;
                            g_free(sranges);
                            goto exit;
                        }

                        if ((sranges[i].saddr & (uint64_t)DWORD_ALIGN_MASK) ||
                             (sranges[i].nbyte & (uint32_t)DWORD_ALIGN_MASK)) {
                            ret = NVME_INVALID_FIELD | NVME_DNR;
                            g_free(sranges);
                            goto exit;
                        }

                        memcpy((uint8_t *)&dest_ns->slm_buf[sb_offset],
                               (uint8_t *)&ns->slm_buf[sranges[i].saddr],
                               copy_size);
    ...
    case NVME_COPY_FORMAT_3:
        ...
        if (NVME_COPY_FORMAT_2 == desc_format) {
            len = sizeof(NvmeCopySourceRangeFormat0_2);
        } else if (NVME_COPY_FORMAT_3 == desc_format) {
            len = sizeof(NvmeCopySourceRangeFormat1_3);
        }

        iocb = qemu_aio_get(&nvme_slm_copy_aiocb_info, NULL,
                                    nvme_misc_cb, req);

        iocb->ranges = g_malloc0(nr  * len);
        ret = nvme_h2c(n, (uint8_t *)iocb->ranges, (nr  * len), req);
        if (ret) {
        ...
        iocb->zone = NULL;
        iocb->format = desc_format;
        iocb->sdaddr_offset = sb_offset;
        iocb->req = req;
        iocb->ret = 0;
        iocb->nr = nr;
        iocb->idx = 0;
        iocb->n = n;
        iocb->dns = dest_ns;
        iocb->copy_length = copy_length;
        iocb->bounce = g_malloc0(SLM_MSSRL_LENGTH);
...
invalid:
        ...
        return ret;
    default:
        printf("%s: range descriptor format not supported %d\n", __func__,
                     desc_format);
        ret = NVME_INVALID_FIELD | NVME_DNR;
        break;
    }
...
static uint16_t nvme_io_cmd_slm(NvmeCtrl *n, NvmeRequest *req)
{
    if (!req->ns->params.slm ||
        !(nvme_cse_iocs_slm[req->cmd.opcode] & NVME_CMD_EFF_CSUPP)) {
        trace_pci_nvme_err_invalid_opc(req->cmd.opcode);
        return NVME_INVALID_OPCODE | NVME_DNR;
    }
    ...
    switch (req->cmd.opcode) {
        ...
        case NVME_CMD_SLM_COPY:
            return nvme_slm_copy(n, req);
    }

    g_assert_not_reached();
}

static uint16_t nvme_io_cmd_compute(NvmeCtrl *n, NvmeRequest *req)
{
    if (!req->ns->params.compute ||
        !(nvme_cse_iocs_compute[req->cmd.opcode] & NVME_CMD_EFF_CSUPP)) {
        return NVME_INVALID_OPCODE | NVME_DNR;
    }

    switch (req->cmd.opcode) {
        /* TODO: missing support for NVME_CMD_FLUSH */
        case NVME_CMD_EXECUTE_PROGRAM:
            return nvme_compute_exec_prgm(n, req);
    }
...
static uint16_t nvme_io_cmd(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    case NVME_CSI_SLM:
        return nvme_io_cmd_slm(n, req);
    case NVME_CSI_COMPUTE:
        return nvme_io_cmd_compute(n, req);
    }
                                 ...
                                 uint64_t off, NvmeRequest *req)
{
    ...
    case NVME_CC_CSS_ALL:
        ...
        case NVME_CSI_ZONED:
            ...
            break;
        case NVME_CSI_SLM:
            iocs = nvme_cse_iocs_slm;
            break;
        case NVME_CSI_COMPUTE:
            iocs = nvme_cse_iocs_compute;
            break;
        }
        break;
    }
...
static uint16_t nvme_downloadable_type_list(NvmeCtrl *n, uint8_t rae,
    uint32_t buf_len, uint64_t off, NvmeRequest *req)
{
    uint32_t nsid = le32_to_cpu(req->cmd.nsid);
    DownloadableTypeList log = {0};
    NvmeNamespace *ns = nvme_ns(n, nsid);
    uint32_t num_desc = ns->down_prgm_type_list_cnt;
    log.numd = num_desc;
    uint32_t descr_size = sizeof(DownloadableTypeDescrDS);
    uint8_t *list = g_malloc0((descr_size * num_desc) + 32);
    uint32_t trans_len;

    memcpy(list, &log, 32);
    for (int i = 0; i < num_desc; i++) {
        memcpy(list + 32 + (i * descr_size),
        &ns->downloadable_type_list[i], descr_size);
    }

    uint32_t log_page_size = num_desc * descr_size + 32 ;
    trans_len = MIN(log_page_size - off, buf_len);
                                     ...
                                     bool active)
{
    ...
    if (c->csi == NVME_CSI_NVM) {
        ...
        return nvme_c2h(n, (uint8_t *)ns->id_ns_slm, sizeof(NvmeIdNsSLM),
                        req);
    } else if (c->csi == NVME_CSI_COMPUTE && ns->csi == NVME_CSI_COMPUTE) {
        return nvme_c2h(n, (uint8_t *)ns->id_ns_compute,
                        sizeof(NvmeIdNsCompute), req);
    }
...
static uint16_t nvme_format(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    uint32_t nsid = le32_to_cpu(req->cmd.nsid);
    NvmeNamespace *ns = nvme_ns(n, nsid);
    if (!(n->id_ctrl.fna & 1) && (ns->csi == NVME_CSI_COMPUTE)) {
...
static uint16_t nvme_program_act_mgmt(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    case 0x1:
    {
        ...
        case 0xC1:
        {
            ...
            g_autofree char *crt_dir = g_new(char, 30 + n_htp);
            ...
            g_autofree char *s = g_new(char, 30 + n_htp);
...
static void nvme_init_iocs_oncs(NvmeCtrl *n, uint32_t *iocs, uint16_t oncs)
{
    uint32_t cmd_val;
    ...
    cmd_val =
        oncs & NVME_ONCS_RESERVATIONS ?
        NVME_CMD_EFF_CSUPP : 0;
    iocs[NVME_CMD_RSV_REGISTER] = cmd_val;
    iocs[NVME_CMD_RSV_REPORT] = cmd_val;
    iocs[NVME_CMD_RSV_ACQUIRE] = cmd_val;
    iocs[NVME_CMD_RSV_RELEASE] = cmd_val;
