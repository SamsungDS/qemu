...
static void nvme_fdp_ruh_change_ru(NvmeCtrl *n, NvmeNamespace *ns,
                                   NvmeRuHandle *ruh)
{
    NvmeFdpEvent *e = NULL;
    ...
    if (ruh->nlb_ruamw) {
        if (log_event(ruh, FDP_EVT_RU_NOT_FULLY_WRITTEN)) {
            uint16_t ruhid = nvme_phid_to_ruhid(ns, ruh->ruhid);

            e = nvme_fdp_alloc_event(n, &ns->fdp.host_events);
            e->type = FDPEF_PIV | FDPEF_NSIDV | FDPEF_LV;
            e->type = FDP_EVT_RU_NOT_FULLY_WRITTEN;
            e->pid = cpu_to_le16(nvme_ruh_pid(ns, ruh->rgid, ruhid));
            e->nsid = cpu_to_le32(ns->params.nsid);
            e->rgid = cpu_to_le16(ruh->rgid);
            e->ruhid = ruh->ruhid;
        }

        /* log (eventual) GC overhead of prematurely swapping the RU */
        nvme_fdp_stat_inc(&ns->fdp.mbmw, nvme_l2b(ns, ruh->nlb_ruamw));
    }
    ruh->nlb_ruamw = ns->fdp.ru_nlba;
    ruh->nlb_deallocated = 0;
}
                              ...
                              uint32_t nlb)
{
    ...
    uint16_t dspec = le16_to_cpu(rw->dspec);
    uint16_t ruh_ndx;
    NvmeRuHandle *ruh;

    if (dtype != NVME_DIRECTIVE_FDP) {
        /*
         * Use PHID 0, but can freely choose which RG to use
         * for now, always send to RG0
         */
        ruh_ndx = nvme_phid_to_ruhid(ns, 0);
    } else {
        ruh_ndx = nvme_fdp_ruhs_offset(ns, dspec);
    }

    nvme_fdp_stat_inc(&ns->fdp.hbmw, data_size);
    nvme_fdp_stat_inc(&ns->fdp.mbmw, data_size);

    ruh = &ns->fdp.ruhs[ruh_ndx];
    trace_pci_nvme_fdp_ruh_write(ruh->rgid, ruh->ruhid, ruh->nlb_ruamw, nlb);
    while (nlb) {
        if (nlb < ruh->nlb_ruamw) {
            ruh->nlb_ruamw -= nlb;
            break;
        } else {
            nlb -= ruh->nlb_ruamw;
            trace_pci_nvme_fdp_ruh_change(ruh->rgid, ruh->ruhid);
            nvme_fdp_ruh_change_ru(n, ns, ruh);
        }
    }
}
                                       ...
                                       size_t len)
{
    ...
    nruhsd = ns->fdp.nrg * ns->fdp.nruhrg;
    ...
    for (i = 0; i < nruhsd; i++, ruhsd++, ruhs++) {
        ruhsd->pid = nvme_ruh_pid(ns, ruhs->rgid, ruhs->ruhid);
        ruhsd->ruhid = ruhs->ruhid;
        ruhsd->earutr = 0;
        ruhsd->ruamw = ruhs->nlb_ruamw;
    }
...
static uint16_t nvme_io_mgmt_send_ruh_update(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    uint16_t npid = ((uint16_t)(cdw10 >> 16)) + 1;
    unsigned int i = 0;
    uint16_t *pids = NULL;

    if (unlikely(nvme_fdp_num_ruhs(ns) < npid)) {
        return NVME_INVALID_FIELD | NVME_DNR;
    }

    pids = g_new0(uint16_t, npid);
    ...
    for (; i < npid; i++) {
        ruh = &ns->fdp.ruhs[nvme_fdp_ruhs_offset(ns, pids[i])];
        nvme_fdp_ruh_change_ru(n, ns, ruh);
    }
                               ...
                               uint64_t off, NvmeRequest *req)
{
    ...
    /* TODO: assume endgrpid == nsid (1 endurance group per NS) */
    ns = nvme_ns(n, endgrpid);
    if (!ns) {
        return NVME_INVALID_FIELD | NVME_DNR;
    }
    ...
    ruhd = (NvmeRuhDescr *) (buf + sizeof(NvmeFdpConfsHdr)
    ...
    if (ns->fdp.enabled) {
        ...
        hdr->fdpa = FIELD_DP8(hdr->fdpa, FDPA, RGIF, ns->fdp.rgif);
        hdr->nrg = cpu_to_le16(ns->fdp.nrg);
        hdr->nruh = cpu_to_le16(ns->fdp.nruhrg);
        hdr->no_nss = cpu_to_le32(1);
        hdr->runs = cpu_to_le64(ns->fdp.ru_nlba * ns->lbasz);

        for (i = 0; i < nruh; i++) {
            ruhd->ruht = RUHT_INITIALLY_ISOLATED;
            ruhd++;
        }
    } else {
    ...
    ret = nvme_c2h(n, (uint8_t *)buf + off, trans_len, req);
    g_free(buf);
    return ret;
}
                                   ...
                                   NvmeRequest *req)
{
    ...
    ruhs = ns->fdp.ruhs;
    nruhsd = ns->fdp.nrg * ns->fdp.nruhrg;
    log_size = sizeof(NvmeRuhuLog) + nruhsd * sizeof(NvmeRuhuDescr);
    trans_len = MIN(log_size - off, buf_len);
    ...
    hdr->nruh = nruhsd;
    for (i = 0; i < nruhsd; i++, ruhud++, ruhs++) {
        /*
         * keep CTRL_SPECIFIED unset, pretend all RUHs were designated by
         * whomever made the NS
         */
        ruhud->ruha = FIELD_DP8(0, RUHA, HOST_SPECIFIED, 1);
    }
...
{
    NvmeNamespace *ns;
    uint32_t trans_len = MIN(sizeof(NvmeFdpStatsLog) - off, buf_len);
    g_autofree NvmeFdpStatsLog *buf = g_malloc0(sizeof(NvmeFdpStatsLog));
    ...
    buf->hbmw[0] = cpu_to_le64(ns->fdp.hbmw);
    buf->mbmw[0] = cpu_to_le64(ns->fdp.mbmw);
    buf->mbe[0] = cpu_to_le64(ns->fdp.mbe);
                                ...
                                NvmeRequest *req)
{
    ...
    NvmeNamespace *ns = nvme_ns(n, endgrpid);
    if (!ns) {
        return NVME_INVALID_FIELD | NVME_DNR;
    }
    ...
    if (host_events) {
        ebuf = &ns->fdp.host_events;
    } else {
        ebuf = &ns->fdp.ctrl_events;
    }

    log_size = sizeof(NvmeFdpEventsLog)
               + ebuf->nelems * sizeof(NvmeFdpEvent);
    trans_len = MIN(log_size - off, buf_len);
    ...
    ret = nvme_c2h(n, (uint8_t *)elog + off, trans_len, req);
    g_free(elog);
    return ret;
}
                                ...
                                uint32_t *result)
{
    ...
    /* TODO: assume endurance group id matches namespace id */
    ns = nvme_ns(n, endgrpid);
    if (unlikely(!ns)) {
        return NVME_INVALID_FIELD | NVME_DNR;
    }

    feat_fdp = FIELD_DP16(feat_fdp, FEAT_FDP, FDPE, 1);
    feat_fdp = FIELD_DP16(feat_fdp, FEAT_FDP, CONF_NDX, 1);
    *result = feat_fdp;
    trace_pci_nvme_fdp_get_feature_fdp(*result);
    return NVME_SUCCESS;
}
                                            ...
                                            NvmeRequest *req, uint32_t *result)
{
     ...
     * 1. The returned data structure is a plain list, we have no explicit
     *    length attr.
     * 2. There's o way to zero out a supported event entry to indicate that
     *    no further valid items were returned.
     *
     * Basically, the host requests `noet` entries, but we may not have that
     * many to serve and there's no way to communicate this.
     */
    ...
    ruhid = nvme_phid_to_ruhid(ns, phid);
    if (unlikely(ruhid >= nvme_fdp_num_ruhs(ns))) {
        return NVME_INVALID_FIELD | NVME_DNR;
    }
    ...
    event_filter = ns->fdp.ruhs[ruhid].event_filter;
    ...
    for (uint8_t event_type = 0; event_type < FDP_EVT_MAX; event_type++) {
        ...
        s_event->evta = (event_filter >> shift) & 0x1;
...
static uint16_t nvme_set_feature_fdp_events(NvmeCtrl *n, NvmeNamespace *ns,
                                            NvmeRequest *req)
{
    /* TODO: cannot divine from docs that NSID is set and used.. is it..? */
    NvmeCmd *cmd = &req->cmd;
    uint32_t cdw11 = le32_to_cpu(cmd->cdw11);
    uint16_t phid = cdw11 & 0xffff;
    uint16_t ret, ruhid;
    uint8_t noet = ((cdw11 >> 16) & 0xff);
    uint8_t events_enable = le32_to_cpu(cmd->cdw12) & 0x1;
    ...
    ruhid = nvme_phid_to_ruhid(ns, phid);
    if (ruhid >= nvme_fdp_num_ruhs(ns)) {
        return NVME_INVALID_FIELD | NVME_DNR;
    }

    ruh = &ns->fdp.ruhs[ruhid];
...
static uint16_t nvme_directive_receive(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    case NVME_DIRECTIVE_IDENTIFY:
        ...
        case NVME_DIRECTIVE_RETURN_PARAMS:
            if (ns->fdp.enabled) {
                id.supported |= 1 << NVME_DIRECTIVE_FDP;
                id.enabled |= 1 << NVME_DIRECTIVE_FDP;
                id.persistent |= 1 << NVME_DIRECTIVE_FDP;
            }
